...
const PRESET_TONES = [
  { id: 1, name: 'Gentle Waves', description: 'Ocean sounds with soft melody', duration: '2:30' },
  { id: 2, name: 'Forest Dawn', description: 'Bird songs with nature ambience', duration: '3:15' },
  { id: 3, name: 'Soft Piano', description: 'Peaceful piano progression', duration: '2:45' },
  { id: 4, name: 'Wind Chimes', description: 'Gentle chimes with breeze', duration: '3:00' },
  { id: 5, name: 'Ambient Bells', description: 'Tibetan singing bowls', duration: '4:00' },
];

export function AudioCustomization({ config, onChange, onHaptic }: AudioCustomizationProps) {
  const [activeTab, setActiveTab] = useState<'presets' | 'upload' | 'create'>('presets');
  ...
  const playPreview = async (toneId: number) => {
    ...
    if (isPlaying === toneId) {
      ...
      if (result.success) {
        console.log(`✅ ESP32: Playing preset tone ${toneId}`);
      } else {
        console.error(`❌ ESP32: Failed to play tone ${toneId}:`, result.error);
      }
  ...
  const selectPreset = async (toneId: number) => {
    ...
    const result = await esp32Controller.playPresetTone(toneId);
    if (result.success) {
      console.log(`✅ ESP32: Selected preset tone ${toneId}`);
    } else {
      console.error(`❌ ESP32: Failed to select tone ${toneId}:`, result.error);
    }
  };
  ...
  const handleVolumeChange = async (volume: number) => {
    ...
    const result = await esp32Controller.setVolume(volume);
    if (result.success) {
      console.log(`✅ ESP32: Volume set to ${volume}%`);
    } else {
      console.error(`❌ ESP32: Failed to set volume:`, result.error);
    }
  };
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    >
                      ...
                      <p className="text-slate-500 text-xs mt-1">{tone.duration}</p>
                    </motion.button>
          ...
          {activeTab === 'upload' && (
            ...
            >
              <div className="border-2 border-dashed border-slate-600 rounded-xl p-8 text-center">
                ...
                <h3 className="text-white font-medium mb-2">Upload Custom Audio</h3>
          ...
          {activeTab === 'create' && (
            ...
            >
              <div className="text-center py-8">
                ...
                <h3 className="text-white font-medium mb-2">Create Custom Tone</h3>
