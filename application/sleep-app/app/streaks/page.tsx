...
import { Button, Card as HCard, CardBody, CardHeader, Divider, Card } from "@heroui/react";
...
export default function StreaksPage() {
    ...
    const daysInMonth = useMemo(() => {
        ...
        for (let d = 1; d <= lastDay.getDate(); d++) {
            ...
            const log = visibleMock.find((l) => l.date === dateStr) || logs.find((l) => l.date === dateStr);
            days.push({
                date,
        ...
        <main className="flex-1 p-4 pb-8">
            <header className="mt-4 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-neutral-400 mt-1">Track your optimal sleep consistency</p>
                </div>
                    ...
                    <CardBody>
                        <div className="grid grid-cols-7 gap-1">
                            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
                                <div key={d} className="text-center text-sm text-neutral-400 py-2">
                                    {d}
                                </div>
                            ))}
                            ...
                            {daysInMonth.map((cell, i) =>
                                cell ? (
                                    <button
                                        ...
                                        className={`aspect-square p-2 rounded-lg relative ${cell.hasOptimumSleep
                                        ...
                                        title={
                                            cell.sleepDuration
                                                ? `Sleep: ${cell.sleepDuration}h, Quality: ${cell.sleepQuality}/10${cell.hasOptimumSleep ? " (Optimal)" : " (Sub-optimal)"}`
                                                : "No data"
                                        }
                                    >
                                        <span className={`text-sm ${cell.date.toISOString().slice(0, 10) === todayStr ? "font-bold" : ""}`}>
                                            {cell.date.getDate()}
                                        </span>
                                    </button>
                                ) : (
                        ...
                        {selected && (
                            <div className="mt-4 p-3 bg-neutral-900 rounded-lg border border-white/6">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <div className="text-sm text-neutral-400">{selected.date}</div>
                                        <div className="text-lg font-medium">{selected.sleepDuration ? `${selected.sleepDuration}h` : "No data"}</div>
                                        <div className="text-sm text-neutral-400">{selected.sleepQuality ? `Quality: ${selected.sleepQuality}/10` : ""}</div>
                                    </div>
                                    <div className="flex items-center gap-2">
