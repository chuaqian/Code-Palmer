...
export function SleepCycleVisualization({
  ...
  const calculateOptimalBedtimes = (): SleepRecommendation[] => {
    ...
    for (let cycles = 4; cycles <= 6; cycles++) {
      let bedtimeMinutes =
        wakeTimeMinutes - (cycles * sleepCycleDuration + fallAsleepTime);
  ...
  const getSleepQuality = (cycles: number) => {
    if (cycles >= 5) return { label: "Optimal", color: "green" } as const;
    if (cycles >= 4) return { label: "Good", color: "yellow" } as const;
    return { label: "Short", color: "red" } as const;
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-purple-500/10 border border-purple-500/20 rounded-xl p-4">
        ...
        <p className="text-slate-300 text-sm leading-relaxed">
          ...
          helps you feel more refreshed and less groggy.
        </p>
      </div>
        ...
        {recommendations.map((rec, index) => {
          ...
          return (
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              onClick={() =>
                onSelectTime({ hour: rec.hour, minute: rec.minute })
              }
              className={`
                ...
                <div className="flex items-center gap-4">
                  <div className="text-left">
                    <div className="text-2xl font-light text-white">
                      {String(rec.hour).padStart(2, "0")}:
                      {String(rec.minute).padStart(2, "0")}
                    </div>
...
export default SleepCycleVisualization;
