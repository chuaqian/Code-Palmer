...
export function AudioCustomization({
  ...
  const playPreview = async (toneId: number) => {
    ...
    if (isPlaying === toneId) {
      ...
      setIsPlaying(toneId);
      // Optional backend integration: preview tone if supported
      try {
        const c: any = esp32Controller as any;
        if (typeof c.playPresetTone === "function") {
          const result = await c.playPresetTone(toneId);
          if (result?.success) {
            console.log(`Preview preset tone ${toneId}`);
          } else if (result && result?.success === false) {
            console.error(`Failed to play tone ${toneId}:`, result.error);
          }
        } else if (typeof c.testBuzzer === "function") {
          // Fallback quick beep
          await c.testBuzzer();
        } else {
          console.log("Preview tone (frontend only):", toneId);
        }
      } catch (e) {
        console.warn("Preview tone error (ignored):", e);
      }
...
export default AudioCustomization;
